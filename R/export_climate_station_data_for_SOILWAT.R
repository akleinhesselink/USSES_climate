...
if(!dir.exists('data/processed_soil_data/weather_files')){ dir.create('data/processed_soil_data/weather_files')}
...
station_dat <- station_dat %>% select( date, STATION, STATION_NAME, PRCP, SNWD, SNOW, TMAX, TMIN )  
...
for ( i in 1:length( year_list) ) { 
  ...
  temp_fname <- file.path( 'data/processed_soil_data/weather_files', paste0( 'weath.', temp_year) )
