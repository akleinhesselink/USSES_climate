...
if(!dir.exists('data/processed_soil_data/soil_files')){ dir.create('data/processed_soil_data/soil_files')}

soil <- readRDS('data/processed_soil_data/decagon_data_with_station_data.RDS')
...
for( i in 1:length(out_list) ) { 
  ...
  write.csv(out_list[[i]], file.path('data/processed_soil_data/soil_files', fname), row.names = FALSE )
}
...
write.csv( sms_labels, 'data/processed_soil_data/soil_files/FieldSensors_MappedTo_SoilWatLayers.csv', row.names = FALSE)
write.table( port_labels, 'data/processed_soil_data/soil_files/port_info.csv', sep = ',', row.names = FALSE)
