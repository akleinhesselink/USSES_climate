...
df$Treatment <- factor(df$Treatment, levels = c('Drought', 'Control', 'Irrigation'), order = T)
...
p_avg <- 
  df %>%
  filter( stat == 'raw', bad_values == 0 ) %>% 
  group_by(Treatment, PrecipGroup, measure, new_date, depth) %>% 
  summarise( v = mean(v,na.rm = TRUE)) %>% 
  group_by(PrecipGroup, measure) %>% 
  do( p = plot_ts_average(x = . )  )
